...
import ec.edu.uce.optativa3.controlador.ConexionPostgres;
...
public class ListarPersonas extends AppCompatActivity {
    ...
    ArrayList<Estudiante> listarPers, listarPorFecha, listarPorApellido;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // datos a firebase
        firebase = FirebaseDatabase.getInstance().getReference("Logs");
        ...
        listaViewUsuarios.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                Estudiante pers = listarPers.get(i);
           ...
           // String usu = extra.getString("usuario");
...
/*
        subir.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Logs tablaLogs;
                String query = "Insert into logs(nombre_usuario,inicio,fin,nombre,modelo,version_cel) values(?,?,?,?,?,?)";
                try {
                    PreparedStatement ps = postgres.conexionDB().prepareStatement(query);
                    for (int j=0;j<base.listarLogs().size();j++) {
                        tablaLogs = base.listarLogs().get(j);
                        ps.setString(1, tablaLogs.getNombreUsuario());
                        ps.setString(2, tablaLogs.getInicio());
                        ps.setString(3, tablaLogs.getFin());
                        ps.setString(4, tablaLogs.getNombre());
                        ps.setString(5, tablaLogs.getModelo());
                        ps.setString(6, tablaLogs.getVersion());
                        ps.executeUpdate();
                    }
                    Toast.makeText(getApplicationContext(),"Los datos se subieron con exito..!!!",Toast.LENGTH_SHORT).show();
...
/*
    public void writeToXmlFile() {
        ArrayList<Estudiante> estudiante = listarPers;
...
*/
    public void agregarEstudiante(View view){
        Intent i = new Intent(ListarPersonas.this,RegistroPersonas.class);
        startActivity(i);
    }

    public void listarPorFecha(View view){
        Intent i = new Intent(ListarPersonas.this,Lista.class);
        startActivity(i);
    }

    public void listarPorApellido(View view){
        Intent i = new Intent(ListarPersonas.this,ListaApellido.class);
        startActivity(i);
    }
    ...
    public void subirDatos(View view){
        ...
        Logs tablaLogs;
        String nombreUsuario, inicio, fin, nombre, modelo, version, id;
        ...
        try {
            ...
            for (int j=0;j<base.listarLogs().size();j++) {
                 tablaLogs = base.listarLogs().get(j);
                    ...
                    id = firebase.push().getKey();
                    nombreUsuario = tablaLogs.getNombreUsuario();
                    inicio = tablaLogs.getInicio();
                    fin = tablaLogs.getFin();
                    nombre = tablaLogs.getNombre();
                    modelo = tablaLogs.getModelo();
                    version = tablaLogs.getVersion();
                    Logs llenar = new Logs(nombreUsuario,inicio,fin,nombre,modelo,version);
                    firebase.child("sesiones").child(id).setValue(llenar);
            }

            Toast.makeText(this,"Los datos se subieron con exito..!!!",Toast.LENGTH_SHORT).show();
        }catch (Exception e){
    ...
    public void fechaFin(){
        ...
        Logs log = base.listarLogsAcceso(usu).get(0);
        String inicio = log.getInicio();

            base.abrirBase();
            base.actualizarLogsAcceso(usu, inicio, fecha);
            base.cerrarBase();
    ...
    public void onBackPressed() {
        ...
        if (contador>=0){
            Toast.makeText(getApplicationContext(),"Debe Cerrar Sesión.....!!!",Toast.LENGTH_SHORT).show();
        }
